...
(function () {
  ...
  /**
   * Scroll top button (NULL-GUARD FIX)
   */
  ...
  function initSwiper() {
    document.querySelectorAll(".init-swiper").forEach(function (swiperElement) {
      ...
      if (swiperElement.classList.contains("swiper-tab")) {
        // If you use custom pagination somewhere:
    ...
    if (window.location.hash) {
      ...
      if (target) {
        setTimeout(() => {
          ...
          window.scrollTo({
            top: target.offsetTop - parseInt(scrollMarginTop || "0", 10),
            behavior: "smooth",
          });
        }, 100);
      }
...
/**
 * Cart badge (conflict-free) — reads from localStorage only
 */
document.addEventListener("DOMContentLoaded", () => {
  ...
  const refreshBadge = () => {
    ...
    cartCountElem.style.display = total > 0 ? "inline" : "none";
  };

  // On load
  refreshBadge();

  // If a page-level add-to-cart exists, refresh after it runs
  const addToCartBtn = document.getElementById("add-to-cart-btn");
  if (addToCartBtn) {
    addToCartBtn.addEventListener("click", () => {
      // Product page script updates localStorage; reflect change immediately
